...
- 顶栏：`0–64px`
- 动态测试主画布：全屏背景
- 主视频画面：左侧 `20,84,500×286`，画面始终等比完整显示；无相机时显示连接状态，不能遮盖动态背景。
- FPS/CPU/GPU/RAM：右侧四张纵向卡片，宽 240px，数值右对齐，任何文字均不得出边界。
- 底部采集与显示状态：`20,384,760×72`，包含 display frame、vsync 和 latest-frame 模式说明。
