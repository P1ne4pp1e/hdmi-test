...
## Revision：指标文字越出卡片（2026-07-12）

**WHAT**：用户反馈仪表盘文字超出边框。

**WHY**：原布局把 CPU、GPU、内存并排塞进三个约 144px 宽的小卡片；内存值的最坏长度无法在该宽度内成立，且文字没有基于实际字体宽度右对齐。

**RULE**：800×480 上每个指标必须在其卡片的内边距范围内完整显示。长数值使用明确缩写单位；标签和数值应分列并由实际字体宽度计算右边界。

**FIX**：右侧改为三张 460×52px 的纵向卡片，所有数值右对齐到 760px，内存改为 GiB 形式；视觉规范已写入精确坐标和最小数值区域。

## Revision：动态背景观感卡顿（2026-07-12）

**WHAT**：用户反馈背景动画不连贯。

**WHY**：客户端仅依赖 EGL swap interval，提交后只处理已经到达的 Wayland 事件，没有等待 Weston 对该帧的 presentation callback。因此客户端帧循环与 compositor 呈现节奏没有明确同步点；同时网格线使用硬阈值，运动时会产生明显跳变。

**RULE**：动态画面每次提交后必须等待对应的 Wayland frame callback，再开始下一帧；连续背景元素必须使用平滑边缘，不能用硬阈值制造非测试目的的闪跳。

**FIX**：每帧在 EGL 提交前登记 `wl_surface_frame` callback，提交并 flush 后阻塞至 Weston 回调，形成明确的显示器节奏；网格从 `step` 改为 `smoothstep`。

进一步采样排除了性能瓶颈：12 秒 `tegrastats` 中 GPU 负载仅 13–17%、温度约 48°C、CPU 负载低且没有热降频。根因是扫描条使用 `fract`，约每 8.3 秒从右边缘瞬移回左边缘，主观上像一次卡顿；现已改为连续正弦波，不再存在周期性重置。

用户进一步指出是持续的低帧率观感，而不是偶发掉帧。为消除 800×480 像素栅格上的视觉混叠，压力背景已移除高频 `fract` 网格，改为三个连续、低频的正弦运动场；客户端每 5 秒写入一次实测 FPS 到服务日志，作为下一步硬件与视觉观测的共同证据。

扫描线是测试画面的必要运动参照，不能移除；保留为高亮纵线，并改为在左右边缘平滑折返，避免原先周期性瞬移。

扫描线仍显得跳动的原因不是掉帧：折返正弦的中心速度曾达到约 4.8px/帧，线宽约 18px，在 800px 面板上会呈现离散移动。已降低至最大约 1px/帧，保持扫描线但让它成为可靠的 60Hz 视觉参照。

根据用户反馈，最终速度调整为最大约 2px/帧：比初版慢一半，但比 1px/帧版本更有明确的连续扫动感。

后续视觉审查确认折返方案在两侧速度降为零，造成“边缘掉帧”的错觉。扫描线现改为恒定速度的循环扫掠（约 3.3px/帧）：当前线离开边缘时下一条线同步从另一侧进入，没有折返、停顿或速度变化。

## Revision：动画运行后逐渐降为低帧率（2026-07-12）

**WHAT**：用户观察到扫描线和背景在运行一段时间后逐渐像 1Hz 一样跳动。

**WHY**：fragment shader 使用 `mediump float` 接收从进程启动以来不断增长的秒数。此精度的尾数有限，运行数分钟后相邻 16.7ms 帧的时间值会量化为相同值或大台阶，导致画面真实帧率仍为 60Hz、动画相位却以低频跳变。

**FIX**：所有动态项调整为 4 秒整周期；C++ 在上传 uniform 前将时间限制到 `[0, 4)`。周期边界上的扫描、波形和对角场相位完全一致，因此无限运行也不会出现精度劣化或可见跳变。
